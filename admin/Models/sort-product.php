...
<?php
    ...
    if (isset($_GET['sorttype'])) {
        $type = $_GET['sorttype'];
        if (isset($_GET['keyword']) && $_GET['c'] == true) {
        ...
        switch ($type) {
            case 'newest':
                $sql = "SELECT * FROM view_products ORDER BY date_created DESC";
                $list = $product->db_get_list($sql);
            break;
            case 'desc':
                $sql = "SELECT * FROM view_products ORDER BY price DESC";
                $list = $product->db_get_list($sql);
            break;
            case 'asc':
                $sql = "SELECT * FROM view_products ORDER BY price ASC";
                $list = $product->db_get_list($sql);
            break;
            default:
                $sql = "SELECT * FROM view_products ORDER BY name ASC";
                $list = $product->db_get_list($sql);
            break;
        }
        }   
